...
if (isset($_POST['pagina']) && isset($_POST['qtd'])) {

$pagina = limpaPost($_POST['pagina']);
$qtd = limpaPost($_POST['qtd']);
$post_inicial = $pagina * $qtd;
//PROCURAR OS POSTS NO BANCO
$sql = $pdo->prepare("SELECT * FROM posts order by id DESC LIMIT $post_inicial, $qtd");
$sql->execute();
...
<div class="card-body">
<h5 class="card-title title">'.$dadosPostAdd['titulo'].'</h5>
<p class="card-text">'.$dadosPostAdd['desc_curta'].'</p>
<a href="artigo.php?x='.$dadosPostAdd['URL'].'" class="btn btn-sm btn-primary btn-post">Ler mais</a>
<a href="categoria.php?x='.$dados_categoria_nome_add[0]['url_categoria'].'" class="btn btn-primary btn-sm btn-post" style="position:relative; float: right;">'.$dados_categoria_nome_add[0]['nome_categoria'].'</a>
